...
#Following load of biomaRt package eed to specify dplyr::select
library(biomaRt)
...
ASDGenes.Brain.SigBPs.MouseKOs <- ASDGenes.Brain.SigBPs.MouseKOPhenotypes %>%
  group_by(UniProtID) %>%
  mutate(nMouseKOPhenotypeX = n_distinct(phenotype.x), nMouseKOPhenotypeY = n_distinct(phenotype.y), nMouseKOPhenotype=(nMouseKOPhenotypeX+nMouseKOPhenotypeY)) %>%
  distinct(UniProtID, .keep_all = TRUE) %>%
...
ASDGenes.DrugTarget <- left_join(ASDGenes.Brain.SigBPs.MouseKO, drugtargets, by='Gene.Name')
```
